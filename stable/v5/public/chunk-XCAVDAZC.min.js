...
`).split(`
`),s="",t="",l=[];for(;r.length>0;){let i=!1,o=[],a;for(a=0;a<r.length;a++)if(this.rules.other.blockquoteStart.test(r[a]))o.push(r[a]),i=!0;else if(!i)o.push(r[a]);else break;r=r.slice(a);let c=o.join(`
`),p=c.replace(this.rules.other.blockquoteSetextReplace,`
    $1`).replace(this.rules.other.blockquoteSetextReplace2,"");s=s?`${s}
${c}`:c,t=t?`${t}
${p}`:p;let u=this.lexer.state.top;if(this.lexer.state.top=!0,this.lexer.blockTokens(p,l,!0),this.lexer.state.top=u,r.length===0)break;let h=l.at(-1);if(h?.type==="code")break;if(h?.type==="blockquote"){let d=h,m=d.raw+`
`+r.join(`
`),x=this.blockquote(m);l[l.length-1]=x,s=s.substring(0,s.length-d.raw.length)+x.raw,t=t.substring(0,t.length-d.text.length)+x.text;break}else if(h?.type==="list"){let d=h,m=d.raw+`
`+r.join(`
`),x=this.list(m);l[l.length-1]=x,s=s.substring(0,s.length-h.raw.length)+x.raw,t=t.substring(0,t.length-d.raw.length)+x.raw,r=m.substring(l.at(-1).raw.length).split(`
...
`,n=n.substring(h.length+1),a=!0),!a){let E=this.rules.other.nextBulletRegex(m),C=this.rules.other.hrRegex(m),S=this.rules.other.fencesBeginRegex(m),y=this.rules.other.headingBeginRegex(m),ot=this.rules.other.htmlBeginRegex(m);for(;n;){let K=n.split(`
`,1)[0],L;if(h=K,this.options.pedantic?(h=h.replace(this.rules.other.listReplaceNesting,"  "),L=h):L=h.replace(this.rules.other.tabCharGlobal,"    "),S.test(h)||y.test(h)||ot.test(h)||E.test(h)||C.test(h))break;if(L.search(this.rules.other.nonSpaceChar)>=m||!h.trim())p+=`
...
`)?"":`
`)+t.raw,i.text+=`
`+t.raw,this.inlineQueue.at(-1).src=i.text):this.tokens.links[t.tag]||(this.tokens.links[t.tag]={href:t.href,title:t.title},r.push(t));continue}if(t=this.tokenizer.table(e)){e=e.substring(t.raw.length),r.push(t);continue}if(t=this.tokenizer.lheading(e)){e=e.substring(t.raw.length),r.push(t);continue}let l=e;if(this.options.extensions?.startBlock){let i=1/0,o=e.slice(1),a;this.options.extensions.startBlock.forEach(c=>{a=c.call({lexer:this},o),typeof a=="number"&&a>=0&&(i=Math.min(i,a))}),i<1/0&&i>=0&&(l=e.substring(0,i+1))}if(this.state.top&&(t=this.tokenizer.paragraph(l))){let i=r.at(-1);s&&i?.type==="paragraph"?(i.raw+=(i.raw.endsWith(`
`)?"":`
`)+t.raw,i.text+=`
